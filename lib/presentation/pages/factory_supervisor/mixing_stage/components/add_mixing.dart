...
import 'package:rice_milling_mobile/presentation/pages/factory_admin/production_planning/models/storage_model.dart';
...
class AddMixingDialog extends StatefulWidget {
  const AddMixingDialog({super.key, this.data});
  final MMixingStage? data;

  @override
  State<AddMixingDialog> createState() => _AddBusinessSettingState();
}

class _AddBusinessSettingState extends State<AddMixingDialog> {
  late MMixingStage termData;

  String? machineIndex;
  List<MMachine> machines = [];

  String? productionIndex;
  List<MProductionPlanning> productionPlannings = [];

  final _ctrlTotalTargetQty = TextEditingController();
  ...
  initData() async {
    await fetchingData();
    if (widget.data == null) {
      termData = MMixingStage.fromJson({});
    } else {
      termData = MMixingStage.copy(widget.data!);
      productionIndex = productionPlannings
          .indexWhere(
              (e) => e.id.toString() == termData.productionBatchId.toString())
          .toString();
      machineIndex = machines
          .indexWhere((e) => e.id.toString() == termData.machineId.toString())
          .toString();
      _ctrlTotalTargetQty.text = termData.totalTargetQty.toString();
  ...
  saveData() async {
    ...
    final productionPlan =
        productionPlannings[int.parse(productionIndex ?? '0')];
    termData.productionBatchId = productionPlan.id;
    termData.machineId = machines[int.parse(machineIndex ?? '0')].id;
    termData.totalTargetQty = productionPlan.totalBatchQty;
    ...
    if (widget.data == null) {
      final res = await AppMixing.insert(termData.toMapCreate());
      if (!res) return;
    } else {
      final res = await AppMixing.update(
  ...
  Future fetchProductionPlanning() async {
    await StoreProductionPlanning.instance.data.initFetch();
    productionPlannings = StoreProductionPlanning.instance.data.datas ?? [];
  }
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      InputSelect(
                        label: l.S.current.stage_production_batch,
                        hintText: l.S.current.action_select,
                        items: productionPlannings
                            .map((e) => e.productionPlanId.toString())
                            .toList(),
                        index: productionIndex,
                        onChange: (value) {
                          productionIndex = value;
                          _ctrlTotalTargetQty.text = productionPlannings[
                                  int.parse(productionIndex ?? '0')]
                              .totalBatchQty
                              .toString();
                          setState(() {});
                        },
                      ...
                      const SizedBox(height: 20),
                      InputText(
                          label: l.S.current.stage_total_target_quantity,
                          hintText: l.S.current.stage_total_target_quantity,
                          keyboardType: TextInputType.number,
                          isDisable: true,
                          controller: _ctrlTotalTargetQty),
                      const SizedBox(height: 20),
                      InputText(
