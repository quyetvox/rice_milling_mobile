...
import 'package:rice_milling_mobile/presentation/pages/factory_supervisor/de_stoning/form_add.dart';
...
import 'models/de_stoning.dart';

class DeStoningView extends StatelessWidget {
  const DeStoningView({super.key});
...
class _DataListViewState extends State<_DataListView> {
  ...
  refresh() async {
    StoreDeStoningQC.instance.data.datas = null;
    setState(() {});
    await Future.delayed(const Duration(milliseconds: 1000), () {});
    await StoreDeStoningQC.instance.data.refresh();
    setState(() {});
  }

  fetchNextPage() async {
    await StoreDeStoningQC.instance.data.fetchNextPage();
    setState(() {});
  }

  void _showFormDialog(BuildContext context, MDeStoningQC? data) async {
  ...
  Widget build(BuildContext context) {
    return BaseView(
      ...
      bodyWidget: StoreDeStoningQC.instance.data.datas == null
          ? const Center(child: CircularProgressIndicator())
          : StoreDeStoningQC.instance.data.datas!.isEmpty
              ? const SizedBox(
                  height: 500,
    ...
    return Theme(
      ...
      child: DataTable(
        ...
        columns: const [
          ...
          DataColumn(label: Text('Brown Rice Input (kg)')),
          DataColumn(label: Text('Stones Removed (kg)')),
          DataColumn(label: Text('Final De-stoned Brown Rice (kg)')),
          DataColumn(label: Text('Start Time')),
          DataColumn(label: Text('End Time')),
          DataColumn(label: Text('Comments')),
          DataColumn(label: Text('Actions')),
        ],
        rows: StoreDeStoningQC.instance.data.datas!.map((data) {
          return DataRow(
            cells: [
              DataCell(Text(data.id.toString())),
              DataCell(Text(data.locationId.toString())),
              DataCell(Text(data.brownRiceInputQty.toString())),
              DataCell(Text(data.stonesRemovedQty.toString())),
              DataCell(Text(data.finalDeStonedBrownRiceQty.toString())),
              DataCell(Text(
