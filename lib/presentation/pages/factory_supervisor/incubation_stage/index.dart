...
import 'components/add_incubation.dart';
import 'models/store_model.dart';

class IncubationStageView extends StatelessWidget {
  const IncubationStageView({super.key});
...
class _DataListViewState extends State<_DataListView> {
  ...
  refresh() async {
    StoreIncubation.instance.data.datas = null;
    setState(() {});
    await Future.delayed(const Duration(milliseconds: 1000), () {});
    await StoreIncubation.instance.data.refresh();
    setState(() {});
  }

  fetchNextPage() async {
    await StoreIncubation.instance.data.fetchNextPage();
    setState(() {});
  }

  void _showFormDialog(BuildContext context, MIncubationStage? data) async {
    final res = await showDialog(
      ...
      builder: (BuildContext context) {
        return BackdropFilter(
            ...
            child: AddIncubationDialog(data: data));
      },
    );
    if (res == null) return;
    if (res) {
  ...
  Widget build(BuildContext context) {
    return BaseView(
      ...
      bodyWidget: StoreIncubation.instance.data.datas == null
          ? const Center(child: CircularProgressIndicator())
          : StoreIncubation.instance.data.datas!.isEmpty
              ? const SizedBox(
                  height: 500,
                  child: Center(
                    child: Text('No data'),
    ...
    return Theme(
      ...
      child: DataTable(
        ...
        columns: [
          ...
          DataColumn(label: Text(l.S.current.stage_actual_temperature)),
          DataColumn(label: Text(l.S.current.stage_actual_moisture_level)),
          DataColumn(label: Text(l.S.current.stage_comments)),
          DataColumn(label: Text(l.S.current.action)),
        ],
        rows: StoreIncubation.instance.data.datas!.map(
          (data) {
            return DataRow(
              ...
              cells: [
                ...
                DataCell(Text(data.actualTemperature.toString())),
                DataCell(Text(data.actualMoistureLevel.toString())),
