...
class AddIncubationDialog extends StatefulWidget {
  const AddIncubationDialog({super.key, this.data});
  final MIncubationStage? data;

  @override
  State<AddIncubationDialog> createState() => _AddBusinessSettingState();
}

class _AddBusinessSettingState extends State<AddIncubationDialog> {
  late MIncubationStage termData;

  String? machineIndex;
  List<MMachine> machines = [];

  String? batchIndex;
  List<MMixingStage> batchs = [];

  //final _ctrlInputQty = TextEditingController();
  final _ctrlActualTemperature = TextEditingController();
  final _ctrlActualMoistureLevel = TextEditingController();
  ...
  initData() async {
    await fetchingData();
    if (widget.data == null) {
      termData = MIncubationStage.fromJson({});
    } else {
      termData = MIncubationStage.copy(widget.data!);
      batchIndex = batchs
          .indexWhere(
      ...
      machineIndex = machines
          ...
          .toString();
      //_ctrlInputQty.text = termData.inputQty.toString();
      _ctrlWastageQty.text = termData.wastageQty.toString();
      _ctrlActualTemperature.text = termData.actualTemperature.toString();
      _ctrlActualMoistureLevel.text = termData.actualMoistureLevel.toString();
  ...
  saveData() async {
    ...
    termData.actualMoistureLevel = double.parse(_ctrlActualMoistureLevel.text);
    termData.actualTemperature = double.parse(_ctrlActualTemperature.text);
    ...
    if (widget.data == null) {
      final res = await AppIncubation.insert(termData.toMapCreate());
      if (!res) return;
    } else {
      final res = await AppIncubation.update(
  ...
  Future fetchMixing() async {
    ...
    batchs = StoreMixing.instance.data.datas ?? [];
  }
              ...
              Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(l.S.current.stage_incubation),
                    IconButton(
                ...
                child: SizedBox(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      // InputText(
                      //     label: 'Input Qty',
                      //     hintText: 'Enter Input Qty',
                      //     keyboardType: TextInputType.number,
                      //     controller: _ctrlInputQty),
                      const SizedBox(height: 20),
                      InputText(
                          label: l.S.current.stage_actual_temperature,
                          keyboardType: TextInputType.number,
                          controller: _ctrlActualTemperature),
                      const SizedBox(height: 20),
                      InputText(
                          label: l.S.current.stage_actual_moisture_level,
                          keyboardType: TextInputType.number,
                          controller: _ctrlActualMoistureLevel),
                      const SizedBox(height: 20),
                      InputText(
