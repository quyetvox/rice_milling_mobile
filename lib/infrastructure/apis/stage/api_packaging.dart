...
class ApiPackaging {
  static Future<List<dynamic>> fetch() async {
    try {
      String url = ('${AppConfig.BASE_URL}/stage/packaging');
      ...
      if (kDebugMode) {
        print("ApiPackaging fetch error: $e");
      }
      return [];
    }
  }
  ...
  static Future<bool> insert(Map<String, dynamic> body) async {
    try {
      ...
      if (kDebugMode) {
        print("ApiPackaging insert error: $e");
      }
      return false;
    }
  }
  ...
  static Future<bool> update(Map<String, dynamic> body, String id) async {
    try {
      String url = ('${AppConfig.BASE_URL}/stage/packaging/$id');
      final response = await ApiRequest.post(url: url, body: jsonEncode(body));
      if (response.statusCode == 200) {
        return true;
      }
      ...
      if (kDebugMode) {
        print("ApiPackaging update error: $e");
      }
      return false;
    }
  }
